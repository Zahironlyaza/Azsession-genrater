...
const readline = require('readline').createInterface({ input: process.stdin, output: process.stdin }); // Readline for local mode
...
app.get('/generate-session', async (req, res) => {
    ...
    waConnect.ev.on('connection.update', async (update) => {
        ...
        if (qr) {
            ...
            // Request a pairing code as well
            try {
                const pairingCode = await waConnect.requestPairingCode("923001234567"); // Use a dummy number or ask user
                console.log('Pairing Code:', pairingCode);
                 res.json({ status: 'qr_code', qr_code: qr, pairing_code: pairingCode });
            } catch (error) {
                console.error('Error generating pairing code:', error);
                 res.json({ status: 'qr_code', qr_code: qr, pairing_code: 'Error generating pairing code. Use QR.' });
            }
        }
    });

    // Handle initial pairing code request if QR is not the first method
    // This is often not needed if QR is shown first, but good for robustness
    if (!waConnect.authState.creds.registered && !res.headersSent) {
        // It's tricky to ask for phone number via HTTP if QR is already sent
        // For a web-based generator, usually QR is primary. Pairing code could be an option for a /pairing-code endpoint
    }
});
